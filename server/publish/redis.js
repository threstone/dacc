...
exports.RedisClientSelf = void 0;
...
class RedisClientSelf {
    ...
    onSubscribeMessage(channel, message) {
     ...
     * @param arglist 参数列表
     */
    reg_timer(timeout, interval, thisObj, func, arglist) {
        return 0;
    }
    stop_timer(timerId) {
        return true;
     ...
     */
    async setData(key, value, sec, time_out) {
        time_out = time_out > 0 ? time_out : DEFAULT_REDIS_TIMEOUT;
     ...
     * @param time_out 请求超时时间
     */
    async hmget(key, field, time_out) {
        time_out = time_out > 0 ? time_out : DEFAULT_REDIS_TIMEOUT;
    ...
    */
     ...
     * @param time_out 请求超时时间
     */
    async hset(key, field, value, sec, time_out) {
        time_out = time_out > 0 ? time_out : DEFAULT_REDIS_TIMEOUT;
        if (typeof value == "object") {
            value = JSON.stringify(value);
        }
     ...
     * @param time_out 请求超时时间
     */
    async setExpire(key, expireTime, time_out) {
        time_out = time_out > 0 ? time_out : DEFAULT_REDIS_TIMEOUT;
     ...
     * @param time_out 请求超时时间
     */
    async sendCommand(command, args, time_out) {
        time_out = time_out > 0 ? time_out : DEFAULT_REDIS_TIMEOUT;
        let self = this;
        return new Promise((resolve, reject) => {
...
exports.RedisClientSelf = RedisClientSelf;
